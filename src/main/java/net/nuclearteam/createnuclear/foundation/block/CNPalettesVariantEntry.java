...
public class CNPalettesVariantEntry {
    ...
    public CNPalettesVariantEntry(String name, CNPalettesStoneTypes cnPalettesVariantEntry) {
        ...
        for (CNPaletteBlockPatten pattern : cnPalettesVariantEntry.variantTypes) {
            BlockBuilder<? extends Block, CreateRegistrate> builder =
                    CreateNuclear.REGISTRATE.block(pattern.createName(name), pattern.getBlockFactory())
        ...
        CreateNuclear.REGISTRATE.addDataGenerator(ProviderType.RECIPE,
                p -> p.stonecutting(DataIngredient.tag(cnPalettesVariantEntry.materialTag), RecipeCategory.BUILDING_BLOCKS,
                        baseBlock));
        CreateNuclear.REGISTRATE.addDataGenerator(ProviderType.ITEM_TAGS, p -> p.addTag(cnPalettesVariantEntry.materialTag)
                .add(baseBlock.get()
                        .asItem()));
